...
public class MovieDao implements iMovieDao {
    ...
    public int insert(Movie mv) {
        ...
        try {
            ...
            String sql = "insert play(play_name, play_length, play_type, play_ticket_price,  play_status, path,play_introduction, ticket_money)values ('"
                    + mv.getPlay_name()
                    + "', "
                    + mv.getPlay_Length()
                    + ", '" +mv.getPlay_type()
                    + "'," + mv.getPlay_ticket_price()+","+mv.getPlay_status()+",'"+mv.getPath()+"','"+mv.getPlay_introduction()+ "',"+mv.getTicket_money()+" )";
            db.openConnection();
    ...
    public List<Movie> select(String condt) {
        ...
        try {
            String sql = "select play_type_id, play_id, play_name, play_introduction, play_ticket_price, play_status, path, play_length,play_type, ticket_money from play ";
            condt.trim();
            if(!condt.isEmpty())
                sql+= " where " + condt;
            DBUtil db = new DBUtil();
            ...
            if (rst!=null) {
                while(rst.next()){
                    ...
                    play.setTicket_money(rst.getInt("ticket_money"));
                    list.add(play) ;
                }
            }
            //关闭数据库对象
    ...
    public int modify(Movie mv) {
        ...
        try{
            String sql = "update play set play_name= '"+mv.getPlay_name()+"'"+
                    ...
                    mv.getPath()+"',ticket_money="+mv.getTicket_money()+"  where play_id = "+mv.getPlay_id();
            DBUtil db = new DBUtil();
            db.openConnection();
            rtn=db.execCommand(sql);
            db.close();
