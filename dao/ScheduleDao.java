...
public class ScheduleDao implements iScheduleDao {
        ...
        try {
            ...
            if (rst!=null) {
                while(rst.next()){
                    ...
                    sched.setSched_time_end(rst.getString("sched_time_end"));
                    //加入到链表
                    list.add(sched);
                }
            }
        ...
        //更新演出厅信息
            ...
            try {
                String sql = "update schedule set " + " studio_id ="
                        ...
                        + s.getSched_ticket_price()+", sched_status="+s.getStatus()+", sched_time_end='"
                        +s.getSched_time_end()+"'";
    ...
    public int add(Schedule sd) {
        try {
            DBUtil db = new DBUtil();
            String sql = "insert schedule (studio_id, play_id, sched_time, sched_ticket_price, sched_status, sched_time_end)values ('"
                    + sd.getStudio_id()
                    + "', "
                    + sd.getPlay_id()
                    + ", '" + sd.getSched_time()
                    + "'," + sd.getSched_ticket_price()
                    + ","+sd.getStatus()+",'"+sd.getSched_time_end()+"')";
            System.out.println(sql) ;
            db.openConnection();
